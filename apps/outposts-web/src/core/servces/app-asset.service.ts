...
export class AppAssetService {
	...
	private _loadScript(
		...
		return new Observable<Event>((observer) => {
			...
			if (targetElement) {
				...
				observer.error(
					`${this.constructor.name} loadScript error, target ${targetElement} not found`,
				);
			}
		});
	}
	...
	private _loadLink(
		url: string,
		attributes: { [attr: string]: string; rel: string },
		targetEl: HTMLElement | string = "head",
	) {
		return new Observable<Event>((observer) => {
			...
			if (targetElement) {
				...
				observer.error(
					`${this.constructor.name} loadLink error, target ${targetElement} not found`,
				);
			}
		});
	}

	private _loadPlainText(url: string) {
	...
	private _getLoaded$(url: string): BehaviorSubject<unknown | undefined> {
		this.loaded$[url] = this.loaded$[url] || new BehaviorSubject(undefined);
	...
	loadScript(
		...
		this.loaders[url] =
			this.loaders[url] ||
			this._loadScript(url, attributes, targetEl).pipe(
				take(1),
				shareReplay(1),
	...
	loadLink(
		...
		this.loaders[url] =
			this.loaders[url] ||
			this._loadLink(url, attributes, targetEl).pipe(
				take(1),
				shareReplay(1),
				tap((value) => this._getLoaded$(url).next(value)),
				catchError((e, caught) => {
	...
	isAssetLoaded$(url: string): Observable<boolean> {
		return this._getLoaded$(url).pipe(map((loaded) => !!loaded));
	}
