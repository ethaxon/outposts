...
describe("hourPlusLevelCronExprValidator", () => {
  it("keeps the empty value as a structural validation error", () => {
    expect(hourPlusLevelCronExprValidator(new FormControl(""))).toEqual({
      emptyCronExpr: true,
    });
  });

  it("returns a translation key rather than a parser error message", () => {
    expect(hourPlusLevelCronExprValidator(new FormControl("not a cron expression"))).toMatchObject({
