...
export class DocSectionComponent implements OnInit {
	...
	isMermaidRequired$: Observable<boolean> = this.data$.pipe(
		map(this.detectMermaid.bind(this)),
	);
	isMermaidLoaded$: Observable<boolean> = this.docService.isMermaidLoaded$();

	isKatexRequired$: Observable<boolean> = this.data$.pipe(
		map(this.detectKatex.bind(this)),
	);
				...
				switchMap(([data, src]) => {
					if (isNil(data) && isNil(src)) {
						return throwError(
							() =>
								new Error(
									"DocSectionComponent Error: can not set both src and data",
								),
						);
					}
					return EMPTY;
