...
} from "@/tools/doc/defs/doc-table-of-contents.defs";
  ...
  updateHeadingsTopValue(elememt: HTMLElement): void {
    ...
    for (const heading of headings) {
      const top = Math.floor(
        heading.getBoundingClientRect().top + this.window.scrollY - this.toleranceThreshold,
      );
  ...
  private getHeadings(element: Element): HTMLHeadingElement[] {
    return Array.from(
      element.querySelectorAll<HTMLHeadingElement>(
        ["h1[id]", "h2[id]", "h3[id]", "h4[id]", "h5[id]", "h6[id]"].join(","),
      ),
    );
  }
}
