...
import { RESIZE_EVENT_DELAY, SCROLL_EVENT_DELAY } from "@/core/defs/delay";
...
export class DocTableOfContentsSpy {
	...
	private getTocTop() {
		return clamp(
			this.tocElement?.getBoundingClientRect?.().top ?? 0,
			0,
	...
	private offsetToSection(
		id: string,
		preferBehavior: "instant" | "smooth" = "instant",
	): void {
		...
		if (body && section) {
			...
			const realBehavior =
				preferBehavior === "smooth" &&
				"scrollBehavior" in this.document.documentElement.style
					? "smooth"
					: "instant";
		...
		this.router.events
			.pipe(
				...
				tap((e) => {
					...
					if (
						id &&
						this.tableOfContentsLoader.tableOfContentsItems.some(
							(item) => item.id === id,
						)
					) {
								...
								mutations.filter((m) => {
									switch (m.type) {
										case "attributes":
											if (
												m.attributeName === "id" &&
												this.tableOfContentsLoader.isHeading(m?.target)
											) {
												return true;
											}
											break;
										case "childList":
											...
											if (
												Array.from(m.addedNodes).some((n) =>
													this.tableOfContentsLoader.isHeading(n),
												)
											) {
												return true;
											}
	...
	private setActiveItemId() {
		...
		if (
			scrollOffset < tableOfContentItems[0].top &&
